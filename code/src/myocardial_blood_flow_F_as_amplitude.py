...
class MyocardialBloodFlow(ComputeQuantity):
    ...
    @staticmethod
    def calculate_fermi_maximum_from_popt(popt: np.ndarray) -> float:
        """
        Calculate the maximum value of the Fermi function from fitted parameters array.

        Args:
            popt (np.ndarray): Array of fitted parameters [F, tau_0, k, tau_d]

        Returns:
            float: Maximum value of the Fermi function
        """
        if len(popt) != 4:
            raise ValueError("popt must contain exactly 4 parameters: [F, tau_0, k, tau_d]")

        F, tau_0, k, tau_d = popt
    ...
    def _fitting_model_function(self,
        ...
        Args:
            ...
            tau_d (float): Delay parameter (parameter to fit)
        ...
        R_F = self.fermi_function(t, F, tau_0, k, tau_d)
    ...
    def fermi_curve_fitting(self,
        ...
        Args:
            ...
            tau_d_init (float): Initial guess for delay parameter (tau_d), default is 1.0
        ...
        try:
            ...
            popt, pcov = curve_fit(
                ...
                p0=[F_init, tau_0_init, k_init, tau_d_init], # initial parameter guesses
            ...
            if plot:
                ...
                series = self.fermi_function(t, popt[0], popt[1], popt[2], popt[3])
            ...
            # popt contains the best-fit parameters [F, tau_0, k, tau_d]
            MBF = popt[0]
    ...
    def compute(self) -> np.ndarray:
        ...
        for i, myo_time_series in enumerate(self.myo_pixel_time_series):
            ...
            mbf_value = self.fermi_curve_fitting(myo_time_series, F_init=1.0, tau_0_init=20.0, k_init=0.1, tau_d_init=1.0, plot=True, pixel_index=i)
