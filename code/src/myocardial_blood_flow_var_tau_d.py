...
class MyocardialBloodFlow(ComputeQuantity):
    ...
    def _fitting_model_function(self,
        ...
        Args:
            ...
            tau_d (float): Delay parameter (fixed to 1.0)

        Returns:
            np.ndarray: The modeled myocardial signal (myo_model) for a single pixel (1D array)
        """

        # 1. Generate the impulse response R_F(t) with tau_d fixed to 1.0
        R_F = self.fermi_function(t, F, tau_0, k, tau_d=1.0)
    ...
    def fermi_curve_fitting(self,
        ...
        try:
            ...
            if plot and popt[0] > 100:
                ...
                series = self.fermi_function(t, popt[0], popt[1], popt[2], tau_d=1.0)
            ...
            # popt contains the best-fit parameters [F, tau_0, k], tau_d is fixed to 1.0
            MBF = self.calculate_fermi_maximum_from_popt(popt, tau_d=1.0)
